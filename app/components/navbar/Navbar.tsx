...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`right-0 bg-white bg-opacity-50 text-sm h-screen z-20 p-3 w-[35%] absolute transition-all ease-in duration-300 font-font-family ${
          navbar
            ? "top-[55px] opacity-100 "
            : "top-[-1490px] md:opacity-100 opacity-0"
        } `}
      >
        <div className="mt-[190px] text-center  bg-opacity-50 leading-8">
          {navLinks.map((item, index) => (
            <div key={index}>
              <Link
                to={item.href}
